        ...
        if (item.variation === "postExtensaoComCatalogo") {
          return (
            <ImageContainer
              ...
              imageUrl={item.items.map((item: any) => item.imagem.url)}
              title={item.primary.titulo ? item.primary.titulo : "vazio"}
              description={
                item.primary.descricao ? item.primary.descricao : "vazio"
              }
              link={"Confira o catálogo completo da Cia de Teatro"}
              button={true}
              linkButton={item.primary.link ? item.primary.link : "/"}
              linkHref={
                item.primary.linkCatalogo ? item.primary.linkCatalogo : "/"
              }
              key={index}
            />
          );
